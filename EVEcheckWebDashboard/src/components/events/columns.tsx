  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-start mx-[12%]">
        <div  className="text-left">
        <div >
          {row.original.checkedInAt === null 
          ? <span className="text-muted-foreground"> não fez check-in </span>
          : formatRelative(new Date(row.original.checkedInAt), new Date(), {locale: ptBR})}
          </div>
        </div>
      </div>
    ),
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="bg-muted text-primary p-2 rounded border" >
            ...
            <DropdownMenuItem
              onClick={() => navigator.clipboard.writeText(AttendeeList.email)}
            >
              <button>Copiar email</button>
              
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem><button>Opcao 1</button></DropdownMenuItem>
            <DropdownMenuItem><button>Opcao 2</button></DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      )
    },
